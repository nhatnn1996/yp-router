...
export const TABLES = {
  ...
  requestDetails: {
    ...
    indexes: [
      ...
      "CREATE INDEX IF NOT EXISTS idx_rd_type ON requestDetails(type)",
    ],
  },
};
...
export function buildCreateTableSqlPg(name, def) {
  const cols = Object.entries(def.columns).map(([k, v]) => {
    ...
    pgType = pgType.replace(/\bINTEGER\s+PRIMARY\s+KEY\s+AUTOINCREMENT\b/gi,
      "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY");
    // REAL → DOUBLE PRECISION
    pgType = pgType.replace(/\bREAL\b/gi, "DOUBLE PRECISION");
    return `${k} ${pgType}`;
  });
  if (def.primaryKey) cols.push(def.primaryKey);
  return `CREATE TABLE IF NOT EXISTS ${name} (${cols.join(", ")})`;
