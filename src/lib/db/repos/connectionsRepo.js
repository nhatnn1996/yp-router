...
function connToRow(c) {
  ...
  const resolvedAuthType = authType ?? c.authtype ?? "apikey";
  const resolvedCreatedAt = createdAt ?? c.createdat ?? new Date().toISOString();
  const resolvedUpdatedAt = updatedAt ?? c.updatedat ?? new Date().toISOString();
  return {
    id,
    provider,
